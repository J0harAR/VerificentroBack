            ...
            if (estadoId) {
                $.ajax({
                    ...
                    success: function (response) {
                        $('#municipio').empty().append('<option value="" disabled selected>Selecciona un municipio</option>');
                        if (response.municipios.length > 0) {
                            $.each(response.municipios, function (key, municipio) {
                                $('#municipio').append('<option value="' + municipio.id + '">' + municipio.description + '</option>');
                            });
                        } else {
                            $('#municipio').append('<option value="" disabled>No hay municipios</option>');
                        }
                    },
                    error: function () {
